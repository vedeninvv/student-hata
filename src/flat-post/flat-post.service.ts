...
export class FlatPostService {
  ...
  async getFlatPostsWithAccountInfo(start: number, end: number): Promise<FlatPostWithAccountInfoDto[]> {
    ...
    for (let i = 0; i < flatPosts.length; i++) {
      ...
      let flatPostWithAccountInfoDto: FlatPostWithAccountInfoDto = {
        ...flatPost,
        ...account,
        preferredUniversities: preferredUniversitiesString,
        undesirableUniversities: undesirableUniversitiesString
      };
